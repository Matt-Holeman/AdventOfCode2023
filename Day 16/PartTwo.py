...
def solve():
    ...
    for x in range(len(mirrorMatrix[0])):
        stepHistory = {}
        travel(mirrorMatrix, stepHistory, x, 0, 0, 1)
        highestScore, bestMap = (len(stepHistory.keys()), stepHistory) if len(stepHistory.keys()) > highestScore else (highestScore, bestMap)

    for x in range(len(mirrorMatrix[0])):
        stepHistory = {}
        travel(mirrorMatrix, stepHistory, x, len(mirrorMatrix), 0, -1)
        highestScore, bestMap = (len(stepHistory.keys()), stepHistory) if len(stepHistory.keys()) > highestScore else (highestScore, bestMap)

    for y in range(len(mirrorMatrix[0])):
        stepHistory = {}
        travel(mirrorMatrix, stepHistory, 0, y, 1, 0)
        highestScore, bestMap = (len(stepHistory.keys()), stepHistory) if len(stepHistory.keys()) > highestScore else (highestScore, bestMap)

    for y in range(len(mirrorMatrix[0])):
        stepHistory = {}
        travel(mirrorMatrix, stepHistory, len(mirrorMatrix), y, -1, 0)
        highestScore, bestMap = (len(stepHistory.keys()), stepHistory) if len(stepHistory.keys()) > highestScore else (highestScore, bestMap)

    visualiseStepHistory(bestMap, mirrorMatrix)
    print(highestScore)
